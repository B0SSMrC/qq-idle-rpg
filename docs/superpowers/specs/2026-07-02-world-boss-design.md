...
The current deepest normal monster is around `2200 HP`, so the world Boss should still be the highest-HP enemy, while keeping each attack visibly meaningful for the current player stats.

Recommended base formula:

```text
boss_hp_max = 9000 + active_player_count * 3000
```

Where `active_player_count` means players in the group who explored or attacked a world Boss in the last `7 days`.
With the current level 18 main players, this puts a single attack at roughly `8%-12%` of a two-player Boss HP pool.
